...
## Key entry points
- [project_entry_points.md](project_entry_points.md) — main: cmd/kvach/main.go -> internal/cli/root.go (Cobra). Loop: agent.Agent.Run() -> agent.loop() (implemented). CLI: kvach run "prompt" wired end-to-end. Tools: tool.DefaultRegistry (process-wide). Profiles: agent.DefaultProfileRegistry (process-wide, RegisterBuiltins at init).
